...
router.post('/analytics-manager/table', async (req, res) => {
    ...
    const { 'x-workspace-id': workspaceId } = req.headers
    let table = `${details.table}${workspaceId}`
    let response = await Dashboard.table({TABLE_NAME: table, orderBykey: details.orderBykey, orderByDirection: details.orderByDirection, limit: details.limit, skipRowby: details.skipRowby})
    // console.log(response)
    res.status(200).send( { status: true, message: "successful", data: response } )
})
